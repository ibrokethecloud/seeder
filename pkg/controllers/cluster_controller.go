...
func (r *ClusterReconciler) wipeDisk(iObj *seederv1alpha1.Inventory) {
	...
	config := &ssh.ClientConfig{
		...
		// WARNING: Do not use InsecureIgnoreHostKey in production.
		// Use ssh.FixedHostKey(key) or parse your ~/.ssh/known_hosts file.
	...
	if err != nil {
		r.Error(err, "failed to dial SSH client", iObj.Name)
		return
	}
	defer func() {
		err := client.Close()
		if err != nil {
			r.Error(err, "failed to close SSH client after wiping disk on node", iObj.Name)
		}
	}()

	session, err := client.NewSession()
	if err != nil {
		r.Error(err, "failed to create session for wiping disk on node", iObj.Name)
		return
	}
	defer func() {
		err := session.Close()
		if err != nil {
			r.Error(err, "failed to close SSH session after wiping disk on node", iObj.Name)
		}
	}()
	...
	for range 3 {
		command := fmt.Sprintf("sgdisk -Z %s", iObj.Spec.PrimaryDisk)
		if err := session.Run(command); err != nil {
			r.Error(err, "command returned error while wiping disk on node", iObj.Name)
			r.Error(fmt.Errorf("stderr output: %s", stderrBuf.String()), "command stderr while wiping disk on node", iObj.Name)
		} else {
			r.V(1).Info("remote ssh stdout:\n%s\n", "node", iObj.Name, stdoutBuf.String())
			return
		}
	}
	r.Error(fmt.Errorf("failed to wipe disk on node after 3 attempts, manual intervention may be needed"), "command failed", iObj.Name)
}
